...
<div class="wrap">
	...
	<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=scalyn-mail-relay-data-controls' ) ); ?>">
		...
		<p id="retention-help"><?php esc_html_e( 'Choose 1–3650 days (default 30). This applies to email logs with their entire timelines, complete diagnostic runs, and health snapshots. Cleanup runs hourly in batches of up to 100 messages, 100 diagnostic runs and 100 snapshots. Records exactly at the cutoff remain. A diagnostic run remains until all its results expire. Existing orphan timeline rows, alerts and audit records are outside this policy.', 'scalyn-mail-relay' ); ?></p>
	...
	<p><?php esc_html_e( 'Rows removed in the last attempt — email logs / timeline events / diagnostic results / health snapshots:', 'scalyn-mail-relay' ); ?> <?php echo esc_html( implode( ' / ', array( $status['mail_logs'], $status['timeline_events'], $status['diagnostic_rows'], $status['health_scores'] ) ) ); ?></p>
